...
public class IndexJoinScan implements Scan {
    ...
    public boolean next() {
        while (hasCurrentLhs) {
            // Return the next matching record in the index, if one exists.
            if (idx.next()) {
                rhs.moveToRid(idx.getDataRid());
                return true;
            }

            // No more index matches for this left-side record.
            // Advance to the next left-side record and probe its key.
            hasCurrentLhs = lhs.next();
